...
const addStudMainBtn = document.getElementById("add-student-main");
const closeAddStudentModal = document.getElementById("close-modal");
const cardGrid = document.getElementById("main-card-grid");
const deleteBtns = document.querySelectorAll("[data-delete-student]");
...
function deleteStudent(studentId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const res = yield fetch(`/student/delete/${studentId}`, {
            method: "DELETE",
            headers: {
                "csrf-token": csrfToken.value,
                "Content-Type": "application/json"
        ...
        if (parsedResponse.success) {
            const cardDiv = document.getElementById(`card-${studentId}`);
            cardDiv.remove();
            sortStudents();
        }
...
const noStudentsInfo = document.getElementById("no-students-info");
...
function createStudentCard(studName, studId) {
    const newDiv = `  <div
  class="
    col-sm-6 col-lg-3
    d-flex
